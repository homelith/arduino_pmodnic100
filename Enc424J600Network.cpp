/*
 ...
 UIPEthernet network driver for Microchip ENC424J600 Ethernet Interface.
...
// set CS to 0 = active
#define CSACTIVE if (!isActive) {SPI.beginTransaction(mySetting);  digitalWrite(ENC424J600_CONTROL_CS,LOW);} isActive = true
// set CS to 1 = passive
#define CSPASSIVE if (isActive) {digitalWrite(ENC424J600_CONTROL_CS,HIGH);  SPI.endTransaction();} isActive = false
...
void Enc424J600Network::writeControlRegister(uint8_t address, uint8_t data) {
	...
	// do the write
	enc_writeOp(ENC424J600_WRITE_CONTROL_REGISTER, address, &data, 1);
}
...
void Enc424J600Network::writeControlRegister16(uint8_t address, uint16_t data) {
	...
	enc_setBank(address);
	// do the write
	enc_writeOp(ENC424J600_WRITE_CONTROL_REGISTER, address&0x1F,(uint8_t*) &data , 2);
}
...
uint8_t Enc424J600Network::readControlRegister(uint8_t address) {
	...
	enc_setBank(address,true);
	// do the write
	enc_readOp(ENC424J600_READ_CONTROL_REGISTER, address, &retval, 1);
	return retval;
}
...
uint16_t Enc424J600Network::readControlRegister16(uint8_t address) {
	...
	enc_setBank(address,true);
	// do the write
	enc_readOp(ENC424J600_READ_CONTROL_REGISTER, address, (uint8_t*)&retval, 2);
...
void Enc424J600Network::writeBitField(uint8_t address, uint8_t data) {
	...
	enc_setBank(address,true);
	// do the write
	enc_writeOp(ENC424J600_BIT_FIELD_SET, address, &data, 1);
}
...
void Enc424J600Network::enc_setBank(uint8_t address, bool keepEnabled) {
	...
	if (((address & BANK_MASK) != bank) && ((address & BANK_MASK) != 0xE0)) {
		...
		switch ((bank)>>5) {
			case 0 :
				enc_SBI(ENC424J600_BANK0_SELECT, keepEnabled);
				break;
			case 1 :
				enc_SBI(ENC424J600_BANK1_SELECT, keepEnabled);
				break;
			case 2 :
				enc_SBI(ENC424J600_BANK2_SELECT, keepEnabled);
				break;
			case 3 :
				enc_SBI(ENC424J600_BANK3_SELECT, keepEnabled);
				break;
		}
	}
}
...
void Enc424J600Network::init(uint8_t* macaddr) {
	...
	#ifdef ENC424J600DEBUG
		DEBUGSERIAL.println("ENC424J600Init");
	#endif

	/* enable SPI */
	pinMode(ENC424J600_CONTROL_CS, OUTPUT);
	digitalWrite(ENC424J600_CONTROL_CS, HIGH);
	SPI.begin();
	...
	if (readControlRegister16(EUDASTL)==0x0000) {
		...
		/*
			...
			unsigned char arpResponse[ARP_RESPONSE_PACKET_SIZE-MAC_ADDR_SIZE] = { 
				...
				0x04,
				0x00, 0x02,
				ENC424J600_MAC0, ENC424J600_MAC1, ENC424J600_MAC2, ENC424J600_MAC3, ENC424J600_MAC4, ENC424J600_MAC5,
		...
		*/
		...
		// 8.5 RECEIVE FILTER TODO!!!
			...
			// broadcast collection filter => enabled
		...
		// configuration LED
		//ENC424J600WCRU(EIDLEDH, 0x06);

		//ENC424J600PhyWrite(PHCON1,PHCON1_PFULDPX);
		// enable reception
		enc_SBI(ENC424J600_ENABLE_RX);
	} else {
		#ifdef ENC424J600DEBUG
	...
	#ifdef ENC424J600DEBUG
		DEBUGSERIAL.println ("ENC424J600Init complete");
	#endif
}

uint16_t Enc424J600Network::readPacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) {
	#ifdef ENC424J600DEBUG
		memblock *packet = handle == RECEIVEBUFFERHANDLE ? &receivePkt : &blocks[handle];
		memaddress start = handle == RECEIVEBUFFERHANDLE && packet->begin + position > RXSTOP_INIT ? packet->begin + position-RXSTOP_INIT+RXSTART_INIT : packet->begin + position;
...
memhandle Enc424J600Network::receivePacket()
{
	...
	if( (readControlRegister(EIRL) & EIR_PKTIF) ){
		...
		// writeControlRegister16(ENC424J600_WRITE_ERXRDPT, nextPacketPtr);
		writePointer(ENC424J600_WRITE_ERXRDPT, nextPacketPtr,true);

		SPI.transfer(ENC424J600_READ_ERXDATA);
		...
		//rxstat = enc_readOp(ENC424J600_READ_ERXDATA, 0);
		//rxstat |= enc_readOp(ENC424J600_READ_ERXDATA, 0) << 8;
		CSPASSIVE;
...
void Enc424J600Network::setERXRDPT() {
	writePointer(ENC424J600_WRITE_ERXRDPT, nextPacketPtr == RXSTART_INIT ? RXSTOP_INIT : nextPacketPtr-1);
}

memaddress Enc424J600Network::blockSize(memhandle handle) {
	return handle == NOBLOCK ? 0 : handle == RECEIVEBUFFERHANDLE ? receivePkt.size : blocks[handle].size;
}

void Enc424J600Network::sendPacket(memhandle handle) {
	...
	{
		enc_writeOp(ENC424J600_BIT_FIELD_CLEAR, ECON1, ECON1_TXRTS);
	}
	*/
...
uint16_t Enc424J600Network::setReadPtr(memhandle handle, memaddress position, uint16_t len) {
	memblock *packet = handle == RECEIVEBUFFERHANDLE ? &receivePkt : &blocks[handle];
	memaddress start = handle == RECEIVEBUFFERHANDLE && packet->begin + position > RXSTOP_INIT ? packet->begin + position-RXSTOP_INIT+RXSTART_INIT : packet->begin + position;

	writePointer(ENC424J600_WRITE_ERXRDPT, start, true);
...
uint16_t Enc424J600Network::writePacket(memhandle handle, memaddress position, uint8_t* buffer, uint16_t len) {
	//DEBUGSERIAL.print("handle: ");
	//DEBUGSERIAL.print(handle);
	...
	writePointer(ENC424J600_WRITE_EGPWRPT,start);
	if (len > packet->size - position) {
		len = packet->size - position;
	}
...
uint8_t Enc424J600Network::readByteTX(uint16_t addr) {
	...
	writePointer(ENC424J600_WRITE_EGPRDPT, addr,false);

	CSACTIVE;

	// issue read command
	SPI.transfer(ENC424J600_READ_EGPDATA);
...
uint8_t Enc424J600Network::readByte(uint16_t addr) {
	...
	writePointer(ENC424J600_WRITE_ERXRDPT, addr,false);
...
void Enc424J600Network::writeByte(uint16_t addr, uint8_t data) {
	...
	writePointer(ENC424J600_WRITE_EGPWRPT, addr,false);

	CSACTIVE;
	// issue read command
	SPI.transfer(ENC424J600_WRITE_EGPDATA);
...
void Enc424J600Network::copyPacket(memhandle dest_pkt, memaddress dest_pos, memhandle src_pkt, memaddress src_pos, uint16_t len, uint8_t buffertype) {
	memblock *dest = &blocks[dest_pkt];
	memblock *src = src_pkt == RECEIVEBUFFERHANDLE ? &receivePkt : &blocks[src_pkt];
	memaddress start = src_pkt == RECEIVEBUFFERHANDLE && src->begin + src_pos > RXSTOP_INIT ? src->begin + src_pos-RXSTOP_INIT+RXSTART_INIT : src->begin + src_pos;
...
void Enc424J600Network::mempool_block_move(memaddress dest, memaddress src, memaddress len, uint16_t bufstart, uint16_t bufend) {
	if (len == 1) {
		...
		DEBUGSERIAL.println(((Enc424J600Network::enc_readOp(ENC424J600_READ_CONTROL_REGISTER, EDMASTH) <<8) | Enc424J600Network::readOp(ENC424J600_READ_CONTROL_REGISTER, EDMASTL)));

		DEBUGSERIAL.print("DMA dest pointer should be: ");
		DEBUGSERIAL.print(dest);
		DEBUGSERIAL.print(" and is: ");

		DEBUGSERIAL.println(((Enc424J600Network::enc_readOp(ENC424J600_READ_CONTROL_REGISTER, EDMADSTH) <<8) | Enc424J600Network::enc_readOp(ENC424J600_READ_CONTROL_REGISTER, EDMADSTL)));

		DEBUGSERIAL.print("DMA len pointer should be: ");
		DEBUGSERIAL.print(len);
		DEBUGSERIAL.print(" and is: ");

		DEBUGSERIAL.println(((Enc424J600Network::enc_readOp(ENC424J600_READ_CONTROL_REGISTER, EDMALENH) <<8) | Enc424J600Network::enc_readOp(ENC424J600_READ_CONTROL_REGISTER, EDMALENL)));
		*/
		 ...
		 */
		//writeControlRegister(ENC424J600_BIT_FIELD_CLEAR, ECON1L, 0x10);
...
void Enc424J600Network::freePacket() {
	setERXRDPT();
}

void Enc424J600Network::readBuffer(uint16_t len, uint8_t* data) {
	CSACTIVE;
	// issue read command
	SPI.transfer(  ENC424J600_READ_ERXDATA);
...
void Enc424J600Network::writeBuffer(uint16_t len, uint8_t* data) {
	CSACTIVE;
	// issue write command
	SPI.transfer(ENC424J600_WRITE_EGPDATA);
...
void Enc424J600Network::phyWrite(uint8_t address, uint16_t data) {
...
uint16_t Enc424J600Network::phyRead(uint8_t address) {
	writeControlRegister(MIREGADRL,address);
...
void Enc424J600Network::clkout(uint8_t clk) {
...
void Enc424J600Network::powerOff() {
	/*
	enc_writeOp(ENC424J600_BIT_FIELD_CLEAR, ECON1, ECON1_RXEN);
	delay(50);
...
void Enc424J600Network::powerOn() {
	/*
	enc_writeOp(ENC424J600_BIT_FIELD_CLEAR, ECON2, ECON2_PWRSV);
	delay(50);
